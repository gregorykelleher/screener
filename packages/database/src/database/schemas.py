...
class CanonicalEquity(Base):
    """
    Database table for the core (canonical) equity data.
    ...
    __tablename__ = "canonical_equities"
    ...
    def __repr__(self):
        return f"<CanonicalEquity(isin={self.isin}, name={self.name}, symbol={self.symbol})>"
...
class FinancialEquity(Base):
    ...
    Fields:
      - isin: Foreign key from canonical table (primary key)
    ...
    isin = Column(String(12), ForeignKey("canonical_equities.isin"), primary_key=True)
    mics = Column(String(255))
    currency = Column(String(10))
    last_price = Column(Numeric)

    # One-to-one relationship with CanonicalEquity
    canonical = relationship("CanonicalEquity", back_populates="financial")
