...
_AUTH_FEEDS: list[tuple[FetchFunc, type]] = [
...
async def resolve() -> AsyncIterable[RawEquity]:
    ...
    tasks = [_resolve_feed(fetcher, model) for fetcher, model in _AUTH_FEEDS]
...
async def _resolve_feed(
    ...
    Args:
        fetcher (FetchFunc): An async function returning a list of raw feed records
            as dictionaries.
...
async def _safe_fetch(
    ...
    exception, logs the error and returns an empty list.
