...
from equity_aggregator.domain.pipeline.transforms._merge import merge
...
def test_merge_cik_logic_mirrors_isin() -> None:
    """
    Simple mirror check for CIK
    """
    raw_equities = [
        RawEquity(
            name="X",
...
def test_merge_share_class_figi_tie_keeps_first_seen() -> None:
    """
    ARRANGE: group with two different FIGIs
    ACT:     merge
    ASSERT:  picks the FIGI of the first record
    """
    raw_equities = [
        RawEquity(
            name="FIRST CORP",
    ...
    assert actual.share_class_figi == "FIGI00000001"
